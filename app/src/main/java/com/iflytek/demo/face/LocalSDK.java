...
public class LocalSDK {
	 ...
	 *            授权码
	 * @param faceMinSize
	 *            设置检测人脸的最小尺寸 小于最小尺寸的人脸将无法被算法检测到
	 * @param faceMaxSize
	 *            设置检测人脸的最大尺寸 大于最大尺寸的人脸将无法被算法检测到
	 * @param sModelPath
	...
	public int CreateDetChannel() {
		...
		String sDetModelPath = new StringBuilder(sModelPath).append(File.separator).append("_configs_dl_traditional.xml").toString();
...
//		TestLog.netE(TAG, "特征通道耗时:" + (System.currentTimeMillis() - start));
    ...
    {  				
    	...
    	//	long startTime = System.currentTimeMillis();
        	...
        	{
        		// 提取检测到的第一张人脸，为最大人脸
        		featureBean.ret = recog.cwGetFiledFeature(iRecogHandle, pFaceBuffer[0].alignedData, pFaceBuffer[0].alignedW,  
        				pFaceBuffer[0].alignedH, pFaceBuffer[0].nChannels, featureBean.btFeature);
        	}
        	else  // 提取probe特征
        	{
        		featureBean.ret = recog.cwGetProbeFeature(iRecogHandle, pFaceBuffer[0].alignedData, pFaceBuffer[0].alignedW,  
        				pFaceBuffer[0].alignedH, pFaceBuffer[0].nChannels, featureBean.btFeature);
        	}   	
	...
	{		
		...
		// 1:1
		verifyBean.ret = recog.cwComputeMatchScore(iRecogHandle, btFeaProbe, iFeaLen, 1, btFeaFiled, iFeaLen, 1, pScores);
	...
	public int CreateAttriHandle() {
		...
		String sAttriModelPath = new StringBuilder(sModelPath).append(File.separator).append("CWR_Config_attri.xml")
				.toString();
		int ret = attri.cwCreateAttributeHandle(sAttriModelPath, pLicence);	
		if (ret >= ERRCODE_MIN) 
		{
			attributeChannel = -1;    // 置为-1表示创建失败
		}
		else
		{
			attributeChannel = ret;
		}
		return ret;
	}
	...
	public int DestoryAttriHandle() {
		int ret = 0;
		if (attributeChannel != -1) {
			ret = attri.cwReleaseAttributeHandle(attributeChannel);
			if (ret == FaceInterface.cw_errcode_t.CW_SDKLIT_OK)
				attributeChannel = -1;
		}
		return ret;
	...
	public static String getErrMsg(int errCode) {
		...
		case FaceInterface.cw_errcode_t.CW_SERVICE_REGISTID_EXIST:
			errMsg = "注册人脸id已存在";
			break; // 注册人脸id已存在
		case FaceInterface.cw_errcode_t.CW_SERVICE_UNREGISTID_NOEXIST:
			errMsg = "反注册人脸的id不存在";
			break; // 反注册人脸的id不存在
		case FaceInterface.cw_errcode_t.CW_SERVICE_RECOG_GROUP_EMPTY:
			errMsg = "检索的底库为空";
			break; // 检索的底库为空
